from langchain_community.document_loaders import (  # импорт асинхронного загрузчика HTML-страниц
    AsyncHtmlLoader,
)
from langchain_community.document_transformers import (  # импорт трансформеров для чистки и конвертации HTML
    BeautifulSoupTransformer,
    Html2TextTransformer,
)

FILE_TO_PARSE = (
    "app/data/links.txt"  # путь к файлу, где лежат ссылки (по одной в строке)
)
...
def getLinks2Parse() -> list:  # функция: читает файл и возвращает список URL-ов
    try:
        with open(FILE_TO_PARSE, "r") as f:  # открываем файл со ссылками
            return [
                link.strip() for link in f.readlines()
...
def asyncLoader(links):  # функция: загружает ссылки, чистит HTML и пишет в .txt
    ...
    bs_transformer = (
        BeautifulSoupTransformer()
    )  # инициализируем трансформер на BeautifulSoup
    ...
    )  # превращаем HTML в plain-text (без ссылок и картинок)
    docs_transformed = html2text.transform_documents(
        docs
    )  # применяем трансформацию ко всем документам

    for idx, doc in enumerate(
        docs_transformed
    ):  # сохраняем каждый очищенный документ в отдельный файл
        with open(f"{DIR_TO_STORE}/document_{idx}.txt", "w+", encoding="utf-8") as f:
            f.write(doc.page_content)  # пишем текст
            print(
                f"File {DIR_TO_STORE}/document_{idx}.txt saved"
