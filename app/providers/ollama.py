...
from langchain_core.prompts import ChatPromptTemplate, MessagesPlaceholder # генератор шаблонов ChatPrompt + плейс-холдер истории
from langchain_core.messages import HumanMessage, AIMessage                # типы сообщений для сохранения чата
from langchain_ollama import OllamaEmbeddings, OllamaLLM                   # обёртки для Ollama-LLM и Ollama-Embeddings
from langchain.chains.combine_documents import create_stuff_documents_chain # “stuff-RAG”: конкатенирует документы в prompt
...
# -------------------- LLM -------------------------------------------------
MODEL_ID = "electromagneticcyclone/t-lite-q:6_k"  # идентификатор локальной модели в Ollama
model = OllamaLLM(model=MODEL_ID, temperature=0.1)  # инициализация LLM с небольшой температурой

# -------------------- Embeddings (тот же, что и при ingest) --------------
EMBED_ID = "mxbai-embed-large"                   # модель эмбеддингов
embedding_function = OllamaEmbeddings(model=EMBED_ID) # обёртка над /v1/embeddings Ollama

# -------------------- Подключаем существующую базу -----------------------
db = Chroma(persist_directory=CHROMA_PATH,            # путь к дисковому индексу
            embedding_function=embedding_function)    # ⚠️ тот же эмбеддер обязателен
...
def query_rag(message: ChatMessage, session_id: str = "") -> str:
    ...
    response_text = document_chain.invoke(
        {
            "context": db.similarity_search(message.question, k=3),  # поиск наиболее похожих 3 чанков
